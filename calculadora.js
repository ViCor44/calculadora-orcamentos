...
async function carregarDadosDoGoogleSheets(tipoEcra) {
    try {
        const urlComCacheBust = LINK_GOOGLE_SHEETS + "&t=" + new Date().getTime();
        const resposta = await fetch(urlComCacheBust);
...
function calcularPrecoFinal() {
    const idSelecionado = document.getElementById('cmbProduto').value;
    if(!idSelecionado) return alert('Por favor, selecione um produto.');
    const prod = produtos.find(p => p.id == idSelecionado);
    ...
    if (prod.tipoCalculo === 'area' || prod.tipoCalculo === 'area_com_extras') {
        ...
        if (larg > 1.27 && alt > 1.27) alert("⚠️ Excede a bobine de 1.27m (Terá emendas).");
        let area = larg * alt;
        ...
        if(horas <= 0) return alert('Insira as horas.');
        let valorHora = prod.preco; 
        if(horas >= 6) valorHora = parseFloat((prod.preco * 0.7058).toFixed(2));
        else if(horas >= 3) valorHora = parseFloat((prod.preco * 0.7941).toFixed(2));
...
function atualizarListaBackendNuvemCorrigida(lista) {
    ...
    alerta.innerHTML = "💡 Preços sincronizados online com o Google Sheets.";
    lista.parentNode.insertBefore(alerta, lista);
    produtos.forEach((p) => {
        let precoExibicao = p.preco ? p.preco.toFixed(2) + "€" : 'Tabela complexa';
        if(p.tipoCalculo === 'pvc_tabela') precoExibicao = "Preço m² Múltiplo";
        if(p.tipoCalculo === 'quantidade_degrau') precoExibicao = "Preço por pacotes";
        lista.innerHTML += `<li><div class="item-info"><span class="badge">${p.categoria}</span><br><strong>${p.nome}</strong><br><small style="color:#718096;">Base: ${precoExibicao}</small></div><div class="acoes"><button class="btn-editar" onclick="abrirMinhaFolhaPrivada()" style="background:#cbd5e0;color:#2d3748;">Ver no Google</button></div></li>`;
    });
}
